...
function uploadChunk(args, id, start, chunk) {
    ...
    return rp(options)
        ...
        .catch(err => {
            if (err.error.error.error_data) {
                console.log("\n Chunck Upload Error: ", err.error.error.error_data);
                let newOffset = {
                    start_offset: err.error.error.error_data.start_offset,
                    end_offset: err.error.error.error_data.start_offset
                };
                return newOffset;
            }
            if (retry++ >= retryMax) {
...
function uploadChainForRetry(buffer, args, res, ids, retryPath, uniqId) {
    ...
    if (res && res.statusCode && res.statusCode == 400) {
        ...
        console.log("\n slice buffer chunk...");
        var chunk = buffer.slice(res.start_offset, res.end_offset); // slice buffer chunk
        return uploadChunk(args, ids[0], res.start_offset, chunk)
...
function facebookApiVideoUpload(args, isRetry) {
    if (args && !isRetry) {
        ...
        return streamToPromise(stream)
            ...
            .then(([buffer, res]) => uploadChainForRetry(buffer, args, res, [res.upload_session_id, res.video_id], isRetry.path, isRetry.uniqId))
            .then(([id, video_id]) => apiFinish(args, id, video_id))
            .then(result => {
        ...
        let stream = fs.createReadStream(args.stream);
        return streamToPromise(stream)
            .then(buffer => Promise.all([buffer]))
            .then(([buffer]) => uploadChainForRetry(buffer, args, isRetry, isRetry.ids, isRetry.path, isRetry.uniqId))
            .then(([id, video_id]) => apiFinish(args, id, video_id))
            .then(result => {
                fs.unlinkSync(isRetry.path);
                return result
